"""
This module contains the MultiModelEnv for simulating multiple gym environments
"""
__license__ = "BSD-3"
__docformat__ = 'reStructuredText'
__author__ = "Nathaniel Pearson, Jared J. Beard"
...
class MultiModelEnv(Env):
    """   
    MultiModelEnv is designed to accept a set of models and mappings (input ref command to mapping module) to/from a global frame

    (We need to make a doc (rst) file for describing the structure of model model env and how to use it.)
    
    For more information see `gym.Env docs <https://www.gymlibrary.dev/api/core/>`_
        
    **States** (dict)
    
    Global representation of state for each env
    ...
    **Actions**
        ...
        - (str) environment to take action in 
        - (Action) Action to take from global frame
    
    **Transition Probabilities**

    See constituent envs
    ...
    **Input**
    ...
    :param state: (dict) initial state
    :param goal: goal
    :param g_obs_space: ....
    :param g_act_space: ....
    :param envs: (list or dictionary) of environment params
    :params mappings: (list or dict) of mappings or environments
    ...
    :param is_master: (bool) if true, overwrite render/logging params of children, *default*: true
    """
    metadata = {"render_modes": ["plot", "print", "none"], "render_fps": 5}

    def __init__(self, *, seed : int = None, params : dict = None):
        super(MultiModelEnv, self).__init__()
    ...
    def reset(self, *, seed: int = None, options: dict = {}):
        ...
        **Deviates from Gym in that it is assumed you can reset 
        ...
        # get mappings from params and instantiate
        # get envs from params and instantiate them

        #use options to overwrite params
            #reset to initial global stateS
        if options != {}:
            for el in options:
    ...
    def step(self, a : int):
        ...
        #select env
        # map current state to env state and rest env
        # map action to env action and step
        # get a tuple from internal step
        # map to global frame 
        # return to user 
        # add step all function???


        # and reset desired env
        # map 
    ...
    def get_actions(self, s : dict):
        ...
        # get global actions
        # foreach model 
        #   call get actions and map back to global frame
    ...
    def reward(self, s : dict, a : int = None, sp : dict = None):
        ...
        #need to figure out reward mapping sitch. 
    ...
    def render(self):
        ...
        # render all envs?.... Will have async issues.
